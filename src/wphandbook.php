...
class WordPressPublisher {
	...
	public function publishContent(string $endpoint, string $markdownContent, string $slug, ?string $parentSlug = null, int $order = 0): array {
		...
		$parentId = 0;
		if (null !== $parentSlug) {
			$parentId = $this->getPageIdBySlug($endpoint, $parentSlug) ?? 0;
			if (0 === $parentId) {
		...
		$existingPage = $this->getPageBySlug( $endpoint, $slug );
		
		if ($existingPage && isset($existingPage['id'])) {
			echo "Existing page found with ID: {$existingPage['id']}. Updating...\n";
			$response = $this->sendRequest("{$endpoint}/{$existingPage['id']}", $postData, 'POST');
		} else {
			echo "Creating new page with slug: {$slug}\n";
			$response = $this->sendRequest($endpoint, $postData, 'POST');
		}
		
		// Update slug to ID map
		if (isset($response['id'])) {
			$this->slugToIdMap[$slug] = $response['id'];
		}
	...
	private function sendRequest(string $endpoint, array $data = [], string $method = 'GET'): array 
	{
		...
		// Only throw exception for error responses that aren't 404s during GET requests
		if ($httpCode >= 400 && !($method === 'GET' && $httpCode === 404)) {
	 ...
	 */
	private function getPageBySlug( string $endpoint, string $slug ): ?array {
		// Check the cache first
		if ( isset( $this->slugToIdMap[$slug] ) ) {
			return [ 'id' => $this->slugToIdMap[ $slug ] ];
		}
		// Query the API for the page
		$endpoint = "{$endpoint}?slug={$slug}&per_page=1&status=any";
	 ...
	 */
	private function getPageIdBySlug( string $endpoint, string $slug ): ?int {
		$page = $this->getPageBySlug( $endpoint, $slug );
		return $page[ 'id' ] ?? null;
	}
}
